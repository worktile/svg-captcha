...
exports.captchaText = function (options) {
	...
	if (options.charPreset || ignoreChars) {
		i = -1;
		...
		const _chars = chars.split(0);
		const _int = parseInt(size / 2);
		while (!(i === _int)) {
			_randomChars = _chars[0][randomInt(0, _chars[0].length - 1)];
			if (out.indexOf(_randomChars) === -1) {
				++i;
				out += _randomChars;
			}
		}
		...
		_chars[1] += "0";
		while (!(i === size - _int)) {
			_randomChars = _chars[1][randomInt(0, _chars[1].length - 1)];
			if (out.indexOf(_randomChars) === -1) {
				++i;
				out += _randomChars;
			}
		}
		return randomStr(out);
	}
};
